      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="our-apps-menu w-max z-40 absolute top-10 rounded-[18px] -right-10 bg-white large:px-14 px-12 gap-10 large:gap-12 flex h-max py-9 large:py-10 "
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
          >
            <div className="flex flex-col w-max large:gap-[35.6px] gap-7">
              {data.ready.map((item: any) => {
